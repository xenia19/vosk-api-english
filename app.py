...
def download_vosk_model():
    ...
    for model_url in urls:
        ...
        try:
            ...
            urllib.request.urlretrieve(model_url, zip_path, timeout=300)
            print("   ✓ Загружена")
...
def load_models_background():
    ...
    try:
        ...
        if VOSK_MODEL_PATH is None:
            LOAD_ERROR = "Failed to download Vosk model"
            print(f"\n❌ {LOAD_ERROR}")
            MODELS_LOADED = False
        else:
            MODELS_LOADED = True
...
def process_audio():
    ...
    try:
        ...
        try:
            ...
            print(f"   ✓ Конвертировано ({original_length}ms → {len(song)}ms)")
        except Exception as e:
